...
Description: This page is only exclusive to only for birthday servations.
 */

class BirthdayRegistrationScreen extends StatefulWidget {
  final Map<String, String> birthdayInfo;

  const BirthdayRegistrationScreen({
    Key? key, 
    required this.birthdayInfo,
  }) : super(key: key);

  @override
  State<BirthdayRegistrationScreen> createState() => _BirthdayRegistrationScreenState();
}

class _BirthdayRegistrationScreenState extends State<BirthdayRegistrationScreen> {
  ...
  void initState() {
    ...
    final nameParts = widget.birthdayInfo['name']?.split(' ') ?? ['', ''];
    ...
    _streetAddressController = TextEditingController(text: widget.birthdayInfo['address'] ?? '');
    _cityController = TextEditingController(text: widget.birthdayInfo['city'] ?? '');
    _provinceController = TextEditingController(text: widget.birthdayInfo['province'] ?? '');
    _phoneController = TextEditingController(text: widget.birthdayInfo['phone'] ?? '+63');
    _peopleController = TextEditingController(text: widget.birthdayInfo['people'] ?? '');
    _dateController = TextEditingController(text: widget.birthdayInfo['date'] ?? '');
    _timeController = TextEditingController(text: widget.birthdayInfo['time'] ?? '');
    _specialRequestController = TextEditingController(text: widget.birthdayInfo['specialRequest'] ?? '');
  }

...
Description: This page is only exclusive to only for social event servations.
 */

class SocialRegistrationScreen extends StatefulWidget {
  final Map<String, String> socialInfo;
...
class _SocialRegistrationScreenState extends State<SocialRegistrationScreen> {
  ...
  void initState() {
    ...
    final nameParts = widget.socialInfo['name']?.split(' ') ?? ['', ''];
    ...
    _streetAddressController = TextEditingController(text: widget.socialInfo['address'] ?? '');
    _cityController = TextEditingController(text: widget.socialInfo['city'] ?? '');
    _provinceController = TextEditingController(text: widget.socialInfo['province'] ?? '');
    _phoneController = TextEditingController(text: widget.socialInfo['phone'] ?? '+63');
    _peopleController = TextEditingController(text: widget.socialInfo['people'] ?? '');
    _dateController = TextEditingController(text: widget.socialInfo['date'] ?? '');
    _timeController = TextEditingController(text: widget.socialInfo['time'] ?? '');
    _specialRequestController = TextEditingController(text: widget.socialInfo['specialRequest'] ?? '');
  }

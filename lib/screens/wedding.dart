...
Description: This page is only exclusive to only for wedding servations.
 */

class WeddingRegistrationScreen extends StatefulWidget {
  final Map<String, String> weddingInfo;

  const WeddingRegistrationScreen({
    Key? key, 
    required this.weddingInfo,
  }) : super(key: key);

  @override
  State<WeddingRegistrationScreen> createState() => _WeddingRegistrationScreenState();
}

class _WeddingRegistrationScreenState extends State<WeddingRegistrationScreen> {
  ...
  void initState() {
    ...
    final nameParts = widget.weddingInfo['name']?.split(' ') ?? ['', ''];
    ...
    _streetAddressController = TextEditingController(text: widget.weddingInfo['address'] ?? '');
    _cityController = TextEditingController(text: widget.weddingInfo['city'] ?? '');
    _provinceController = TextEditingController(text: widget.weddingInfo['province'] ?? '');
    _phoneController = TextEditingController(text: widget.weddingInfo['phone'] ?? '+63');
    _peopleController = TextEditingController(text: widget.weddingInfo['people'] ?? '');
    _dateController = TextEditingController(text: widget.weddingInfo['date'] ?? '');
    _timeController = TextEditingController(text: widget.weddingInfo['time'] ?? '');
    _specialRequestController = TextEditingController(text: widget.weddingInfo['specialRequest'] ?? '');
  }
